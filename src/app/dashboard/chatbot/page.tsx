...
export default function ChatBotPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { from: 'bot', text: 'Sorry, something went wrong.', time: getCurrentTime() }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className={`${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'} flex flex-col h-screen p-4`}>
      ...
      <main className="flex-1 overflow-y-auto border rounded-lg shadow-inner p-4 bg-white dark:bg-gray-800 space-y-4">
        {messages.map((msg, idx) => (
          ...
          >
            {msg.from === 'bot' && <FaRobot className="text-purple-600 dark:text-purple-400 text-xl mr-2" />}
            {msg.from === 'user' && <FaUserCircle className="text-blue-600 dark:text-blue-400 text-xl mr-2" />}
